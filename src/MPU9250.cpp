...
{
  uint8_t c = _bt->readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9250
  return c;
}

  uint8_t MPU9250::getAK8963CID()
{
  uint8_t c = _bt->readByte(AK8963_ADDRESS, WHO_AM_I_AK8963);  // Read WHO_AM_I register for MPU-9250
  return c;
}
...
void MPU9250::accelWakeOnMotion()
{
  ...
  uint8_t c = _bt->readByte(MPU9250_ADDRESS, ACCEL_CONFIG2); // get current ACCEL_CONFIG2 register value
  c = c & ~0x0F; // Clear accel_fchoice_b (bit 3) and A_DLPFG (bits [2:0])  
  c = c | 0x01;  // Set accelerometer rate to 1 kHz and bandwidth to 184 Hz
  _bt->writeByte(MPU9250_ADDRESS, ACCEL_CONFIG2, c); // Write new ACCEL_CONFIG2 register value
  ...
  // can join the I2C bus and all can be controlled by the Arduino as master 
   ...
   _bt->writeByte(MPU9250_ADDRESS, INT_ENABLE, 0x41);   // Enable data ready (bit 0) and wake on motion (bit 6)  interrupt

   // enable wake on motion detection logic (bit 7) and compare current sample to previous sample (bit 6)
   _bt->writeByte(MPU9250_ADDRESS, MOT_DETECT_CTRL, 0xC0);  

   // set accel threshold for wake up at  mG per LSB, 1 - 255 LSBs == 0 - 1020 mg), pic 0x19 for 25 mg
   _bt->writeByte(MPU9250_ADDRESS, WOM_THR, 0x19);
   ...
   */
  _bt->writeByte(MPU9250_ADDRESS, LP_ACCEL_ODR, 0x02);

  c = _bt->readByte(MPU9250_ADDRESS, PWR_MGMT_1);
...
{
  uint8_t temp = 0;
  temp = _bt->readByte(AK8963_ADDRESS, AK8963_CNTL);
...
void MPU9250::readMPU9250Data(int16_t * destination)
{
  ...
  _bt->readBytes(MPU9250_ADDRESS, ACCEL_XOUT_H, 14, &rawData[0]);  // Read the 14 raw data registers into data array
...
void MPU9250::readAccelData(int16_t * destination)
{
  ...
  _bt->readBytes(MPU9250_ADDRESS, ACCEL_XOUT_H, 6, &rawData[0]);  // Read the six raw data registers into data array
  destination[0] = ((int16_t)rawData[0] << 8) | rawData[1] ;  // Turn the MSB and LSB into a signed 16-bit value
  destination[1] = ((int16_t)rawData[2] << 8) | rawData[3] ;  
  destination[2] = ((int16_t)rawData[4] << 8) | rawData[5] ; 
}
...
void MPU9250::readGyroData(int16_t * destination)
{
  ...
  _bt->readBytes(MPU9250_ADDRESS, GYRO_XOUT_H, 6, &rawData[0]);  // Read the six raw data registers sequentially into data array
  destination[0] = ((int16_t)rawData[0] << 8) | rawData[1] ;  // Turn the MSB and LSB into a signed 16-bit value
  destination[1] = ((int16_t)rawData[2] << 8) | rawData[3] ;  
  destination[2] = ((int16_t)rawData[4] << 8) | rawData[5] ; 
}
...
{
  bool test;
  test = (_bt->readByte(AK8963_ADDRESS, AK8963_ST1) & 0x01);
  return test;
}
...
{
  bool test;
  test = (_bt->readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01);
  return test;
}
...
{
  bool test;
  test = (_bt->readByte(MPU9250_ADDRESS, INT_STATUS) & 0x40);
  return test;
}
...
void MPU9250::readMagData(int16_t * destination)
{
  ...
  _bt->readBytes(AK8963_ADDRESS, AK8963_XOUT_L, 7, &rawData[0]);  // Read the six raw data and ST2 registers sequentially into data array
...
int16_t MPU9250::readGyroTempData()
{
  ...
  _bt->readBytes(MPU9250_ADDRESS, TEMP_OUT_H, 2, &rawData[0]);  // Read the two raw data registers sequentially into data array 
...
{  
 ...
 // With the MPU9250, it is possible to get gyro sample rates of 32 kHz (!), 8 kHz, or 1 kHz
  _bt->writeByte(MPU9250_ADDRESS, CONFIG, 0x03);  

 // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
  _bt->writeByte(MPU9250_ADDRESS, SMPLRT_DIV, sampleRate);  // Use a 200 Hz rate; a rate consistent with the filter update rate 
                                                       // determined inset in CONFIG above
 
 // Set gyroscope full scale range
 // Range selects FS_SEL and AFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
  uint8_t c = _bt->readByte(MPU9250_ADDRESS, GYRO_CONFIG); // get current GYRO_CONFIG register value
 // c = c & ~0xE0; // Clear self-test bits [7:5] 
  c = c & ~0x02; // Clear Fchoice bits [1:0] 
  c = c & ~0x18; // Clear AFS bits [4:3]
  c = c | Gscale << 3; // Set full scale range for the gyro
 // c =| 0x00; // Set Fchoice for the gyro to 11 by writing its inverse to bits 1:0 of GYRO_CONFIG
  _bt->writeByte(MPU9250_ADDRESS, GYRO_CONFIG, c ); // Write new GYRO_CONFIG value to register
  
 // Set accelerometer full-scale range configuration
  c = _bt->readByte(MPU9250_ADDRESS, ACCEL_CONFIG); // get current ACCEL_CONFIG register value
 // c = c & ~0xE0; // Clear self-test bits [7:5] 
  c = c & ~0x18;  // Clear AFS bits [4:3]
  c = c | Ascale << 3; // Set full scale range for the accelerometer 
  _bt->writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, c); // Write new ACCEL_CONFIG register value

 // Set accelerometer sample rate configuration
 // It is possible to get a 4 kHz sample rate from the accelerometer by choosing 1 for
 // accel_fchoice_b bit [3]; in this case the bandwidth is 1.13 kHz
  c = _bt->readByte(MPU9250_ADDRESS, ACCEL_CONFIG2); // get current ACCEL_CONFIG2 register value
  c = c & ~0x0F; // Clear accel_fchoice_b (bit 3) and A_DLPFG (bits [2:0])  
...
// Configure device for bias calculation
  ...
  _bt->writeByte(MPU9250_ADDRESS, FIFO_EN, 0x00);      // Disable FIFO
  _bt->writeByte(MPU9250_ADDRESS, PWR_MGMT_1, 0x00);   // Turn on internal clock source
...
// Configure MPU6050 gyro and accelerometer for bias calculation
  ...
  _bt->writeByte(MPU9250_ADDRESS, SMPLRT_DIV, 0x00);  // Set sample rate to 1 kHz
  _bt->writeByte(MPU9250_ADDRESS, GYRO_CONFIG, 0x00);  // Set gyro full-scale to 250 degrees per second, maximum sensitivity
  _bt->writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, 0x00); // Set accelerometer full-scale to 2 g, maximum sensitivity
...
// At end of sample accumulation, turn off FIFO sensor read
  ...
  _bt->readBytes(MPU9250_ADDRESS, FIFO_COUNTH, 2, &data[0]); // read FIFO sample count
  fifo_count = ((uint16_t)data[0] << 8) | data[1];
  packet_count = fifo_count/12;// How many sets of full gyro and accelerometer data for averaging
  
  for (ii = 0; ii < packet_count; ii++) {
    int16_t accel_temp[3] = {0, 0, 0}, gyro_temp[3] = {0, 0, 0};
    _bt->readBytes(MPU9250_ADDRESS, FIFO_R_W, 12, &data[0]); // read data for averaging
...
// Push gyro biases to hardware registers
  _bt->writeByte(MPU9250_ADDRESS, XG_OFFSET_H, data[0]);
  _bt->writeByte(MPU9250_ADDRESS, XG_OFFSET_L, data[1]);
  _bt->writeByte(MPU9250_ADDRESS, YG_OFFSET_H, data[2]);
  _bt->writeByte(MPU9250_ADDRESS, YG_OFFSET_L, data[3]);
  _bt->writeByte(MPU9250_ADDRESS, ZG_OFFSET_H, data[4]);
  _bt->writeByte(MPU9250_ADDRESS, ZG_OFFSET_L, data[5]);
...
// the accelerometer biases calculated above must be divided by 8.
  ...
  _bt->readBytes(MPU9250_ADDRESS, XA_OFFSET_H, 2, &data[0]); // Read factory accelerometer trim values
  accel_bias_reg[0] = (int32_t) (((int16_t)data[0] << 8) | data[1]);
  _bt->readBytes(MPU9250_ADDRESS, YA_OFFSET_H, 2, &data[0]);
  accel_bias_reg[1] = (int32_t) (((int16_t)data[0] << 8) | data[1]);
  _bt->readBytes(MPU9250_ADDRESS, ZA_OFFSET_H, 2, &data[0]);
...
//  _bt->writeByte(MPU9250_ADDRESS, XA_OFFSET_H, data[0]);
//  _bt->writeByte(MPU9250_ADDRESS, XA_OFFSET_L, data[1]);
//  _bt->writeByte(MPU9250_ADDRESS, YA_OFFSET_H, data[2]);
//  _bt->writeByte(MPU9250_ADDRESS, YA_OFFSET_L, data[3]);
//  _bt->writeByte(MPU9250_ADDRESS, ZA_OFFSET_H, data[4]);
//  _bt->writeByte(MPU9250_ADDRESS, ZA_OFFSET_L, data[5]);
...
void MPU9250::SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
  ...
  _bt->writeByte(MPU9250_ADDRESS, SMPLRT_DIV, 0x00);    // Set gyro sample rate to 1 kHz
  _bt->writeByte(MPU9250_ADDRESS, CONFIG, 0x02);        // Set gyro sample rate to 1 kHz and DLPF to 92 Hz
  _bt->writeByte(MPU9250_ADDRESS, GYRO_CONFIG, 1<<FS);  // Set full scale range for the gyro to 250 dps
  _bt->writeByte(MPU9250_ADDRESS, ACCEL_CONFIG2, 0x02); // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
  _bt->writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, 1<<FS); // Set full scale range for the accelerometer to 2 g
...
// Configure the accelerometer for self-test
 ...
 // Configure the gyro and accelerometer for normal operation
   ...
   selfTest[1] = _bt->readByte(MPU9250_ADDRESS, SELF_TEST_Y_ACCEL); // Y-axis accel self-test results
   selfTest[2] = _bt->readByte(MPU9250_ADDRESS, SELF_TEST_Z_ACCEL); // Z-axis accel self-test results
   selfTest[3] = _bt->readByte(MPU9250_ADDRESS, SELF_TEST_X_GYRO);  // X-axis gyro self-test results
   selfTest[4] = _bt->readByte(MPU9250_ADDRESS, SELF_TEST_Y_GYRO);  // Y-axis gyro self-test results
   selfTest[5] = _bt->readByte(MPU9250_ADDRESS, SELF_TEST_Z_GYRO);  // Z-axis gyro self-test results
